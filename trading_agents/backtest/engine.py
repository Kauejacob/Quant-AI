...
class BacktestEngine:
    ...
    def get_rebalance_dates(self) -> List[pd.Timestamp]:
        ...
        if self.rebalance_frequency == 'weekly':
            ...
            raise ValueError(f"Frequência inválida: {self.rebalance_frequency}")
    ...
    def rebalance_portfolio(self, date: pd.Timestamp):
        ...
        if not buy_signals:
            if self.verbose:
                print("   ⚠️ Nenhum sinal de compra")
            return
        
        # Ordena por confiança * score (melhor primeiro)
        buy_signals.sort(
            key=lambda x: (x['confidence'] or 0) * (x['analyst_score'] or 0),
            reverse=True
        )
        ...
        for signal in buy_signals[:15]:  # Máximo 15 posições
            ...
            if trade and self.verbose:
                print(f"   ✅ BUY {trade.shares} {ticker} @ R${price:.2f} "
                      f"({target_pct:.1f}%, conf: {signal['confidence']:.0%})")
        
        # Vende posições que não estão mais nos sinais
        current_tickers = set(self.portfolio.positions.keys())
        signal_tickers = set(s['ticker'] for s in buy_signals)
        
        to_sell = current_tickers - signal_tickers
        
        if to_sell and self.verbose:
            print(f"\n💸 Vendendo posições sem sinal:")
